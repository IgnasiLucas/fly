...
of values between the two ends: 200, 225, 250, 275, and 295. Table `FromClean/summaryStats.txt` holds
the summary statistics from all 25 runs.
...
are left. This is because qiime discards reads shorter than the trimming position.
...
# Conclusion
The program dada2 to merge reads is not adequate, because it requires all reads to be the same length
before merging, as if all target fragments were the same length.

I need to check if the merging process applied by the sequencing services is also afected by this bias.
