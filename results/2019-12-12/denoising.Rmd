...
sequences. They could be a result of inspecific binding of primers. But where to cut? I think
an alignment can help decide what are valid, homologous amplicons and what not.

```{r alignment, cache=TRUE}
# Sequences are in DNA space. Turning them into RNA will afford an alignment that
# takes RNA secondary structure into account.
seqs <- RNAStringSet(DNAStringSet(colnames(SeqTabNoChim)))
alignment <- AlignSeqs(seqs, normPower=0)  # normPower=0 prevents penalization of incomplete
                                           #   columns produced by abundant shorter sequences.
```
