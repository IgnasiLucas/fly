...
What about the 23 sequences without a Phylum? Let's blast them as well, why not?

```{r noPhylum}
query <- row.names(taxa)[is.na(taxa[,'Phylum']) & !(is.na(taxa[,'Kingdom']))]
table(taxa[query,'Kingdom'])
```
These are eukaryotic sequences, probably not well represented in the reference dataset.
Are all eukaryotes in the microbiome so poorly annotated?

```{r kingdoms}
table(taxa[,'Kingdom'])
```
Well, there is only one more eukaryotic sequence, which must have been annotated further. Which one?

```{r eukaryota}
taxa[row.names(taxa)[taxa[,'Kingdom'] == 'Eukaryota' & !(is.na(taxa[,'Phylum']))],]
```

So, a few eukaryotes are present, but poorly annotated. 24 sequences could easily be annotated by
hand. But that's not very good practice for reproducibility...
