...
I think it is more reasonable to filter by relative abundance than by
prevalence. I remove taxa because they don't contribute much to any sample's
microbiome, not of their absence from a number of samples.

```{r filter}
filter <- Prevalence$meanProp >= 1.0e-05 & Prevalence$prevalence > 12
SeqTabClean <- SeqTabNoChim[,filter]
dim(SeqTabClean)
```

Another way of filtering is to use the original counts. In the Phyloseq Preprocessing tutorial they suggest
removing taxa that has not been counted at least 4 times in at least 20% of samples. In
our data set that would leave `r sum(apply(SeqTabClean, 2, function(x){sum(x>3)}) > (0.2*177))`
taxa. I may create such a reduced subset of the data later, after creating the
phyloseq object, but I find that filter too aggressive.
...
tree and include that information in the `phyloseq` object.

The abundance table, `SeqTabClean`, uses the DNA sequence itself as names of
columns. It is more convenient to use a shorter name, and to store the sequences
in a separate object, indexed by the same names. See the [dada2 tutorial](https://benjjneb.github.io/dada2/tutorial.html).
...
```{r ratio}
Lactobacillus <- as.character(dna) %in% row.names(taxa[taxa[,'Genus'] == 'Lactobacillus',])
Acetobacter   <- as.character(dna) %in% row.names(taxa[taxa[,'Genus'] == 'Acetobacter',])
LactoProp <- colSums(PropClean[Lactobacillus,])
AcetoProp <- colSums(PropClean[Acetobacter,])
summary(LactoProp)
summary(AcetoProp)
