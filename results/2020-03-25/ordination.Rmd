...
`r sprintf("%.1f %%", ord.pcoa.raw$values[1,'Rel_corr_eig'])`, and that for axis
2 is `r sprintf("%.1f %%", ord.pcoa.raw$values[2, 'Rel_corr_eig'])`.
...
`r sprintf("%.1f", ord.pcoa.faith2qW1$values[1,'Rel_corr_eig'])` and
`r sprintf("%.1f %%", ord.pcoa.faith2qW1$values[2, 'Rel_corr_eig'])` relative
eigenvalues.
...
@Legendre2012 [p. ] recommends to standardize the response variables to make interpretation
easier. I create a new data frame `Y` with the transformed variables, using only values for
...
jsut as precise as any other, while in reality some abundances are much more variable
...
# It won't make sense to estimate proportions after filtering.
